...
"use strict";
/* ================= solver (runs inside a Web Worker) ================= */
function workerMain() {
...
function runAny(cfg, post) {
  ...
  onmessage = function (e) {
    try {
      const t0 = Date.now();
      const res = runAny(e.data, function (m) {
        postMessage({ type: 'progress', label: m.label, extra: m.extra, nodes: m.nodes, elapsed: Date.now() - t0 });
      });
      res.elapsed = Date.now() - t0;
      postMessage({ type: 'done', mode: e.data.mode, res: res });
    } catch (err) {
