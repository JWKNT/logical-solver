...
function render(){let g=$('a38Grid');g.style.gridTemplateColumns=`repeat(${C},52px)`;g.innerHTML='';
 ...
 for(let i=0;i<R*C;i++){let d=document.createElement('div');d.className='a38-cell '+kind[i]+((state.forcedCells&&state.forcedCells.has(i))?' forced':'')+((state.permitCells&&state.permitCells.has(i))?' has-permit':'');
  if(kind[i]==='clue'){let inp=document.createElement('span');inp.className='a38-clue-entry';inp.contentEditable='true';inp.spellcheck=false;inp.textContent=(clues[i]||[]).join(' ');inp.setAttribute('aria-label',`Numbers at row ${((i/C)|0)+1}, column ${i%C+1}`);inp.onclick=e=>{if(tool!=='clue'||(clues[i]||[]).length)e.stopPropagation()};inp.onkeydown=e=>{if(e.key.length===1&&!/[1-8 ,]/.test(e.key))e.preventDefault();if((e.key==='Backspace'||e.key==='Delete')&&!(inp.textContent||'').trim()){e.preventDefault();delete clues[i];kind[i]='cell';reset();render();return}e.stopPropagation()};inp.oninput=()=>{let nums=(inp.textContent.match(/[1-8]/g)||[]).map(Number);clues[i]=nums;shown=null;state={};status(nums.length?`Number cell contains ${nums.join(', ')}.`:'Type one or more numbers from 1 to 8. Backspace removes them.');};inp.onblur=()=>{inp.textContent=(clues[i]||[]).join(' ')};d.append(inp)}
...
function cfg(){return {R,C,kind,clues,time:+$('a38Time').value,maxSolutions:runMax,ignoreStart:$('a38IgnoreStart').checked}}
...
function finishRun(r,cands){if(r.error)return statusHTML('<span class="bad">'+esc(r.error)+'</span>');if(!r.solutions.length)return statusHTML(r.timed?'<span class="warn">No solution found within the time limit.</span> Raise the limit and try again.':'<span class="bad">No solution exists.</span>');
 if(cands&&(r.timed||r.capped))return statusHTML('<span class="warn">The complete solution set was not enumerated</span> \u2014 exact directed candidates cannot safely be shown. Raise the time limit.');if(cands){state.forcedEdges=A38Engine.commonDirectedEdges(r.solutions);state.forcedCells=new Set([...state.forcedEdges].flatMap(k=>k.split('>').map(Number)));shown=null}else{shown=r.solutions[0];let info=A38Engine.permitInfo(shown,cfg());state={permitCells:info.cells,permitOrdinals:info.ordinals}}render();statusHTML((r.solutions.length===1&&!r.capped&&!r.timed?'<span class="good">Solved \u2014 the solution is unique.</span> ':r.solutions.length===1?'<span class="good">Solved.</span> <span class="warn">Uniqueness not verified \u2014 the time limit was reached.</span> ':'<span class="good">Solved.</span> <span class="warn">'+r.solutions.length+(r.capped?'+':'')+' directed solutions exist'+(r.timed?' (search timed out)':'')+'.</span> ')+(cands?'Blue arrows occur in every solution.':'The directed route and all acquired permits are shown.'))}
...
$('a38IgnoreStart').addEventListener('change',()=>{reset();render()});
