...
function ruleCoralSpine(st, clues) {
  ...
  for (const line of eachSumsLine(st, clues)) {
    ...
    if (!T.length) {
      return { rule: 'Coral spine', contradiction: true, cells: line.cells.slice(),
  ...
  if (!okComps.length) {
    return { rule: 'Coral spine', contradiction: true, cells: [],
  ...
  if (hits.length) {
    ...
    return { rule: 'Coral spine', cells: hits.slice(0, 12),
      text: 'Every shaded cell joins the one coral, which must reach ' + (missed ? missed.why : 'every clued line') + ' \u2014 but ' + hits.slice(0, 6).map(i2 => rc(st, i2)).join(', ') + (hits.length > 6 ? ', \u2026' : '') + ' sit' + (hits.length === 1 ? 's' : '') + ' in a pocket that cannot, so ' + (hits.length === 1 ? 'it holds a digit' : 'they hold digits') + '.',
  ...
  if (okComps.length === 1) {
    for (let x = 0; x < N; x++) {
      ...
      if (!any) {
        return { rule: 'Coral spine', cells: [x],
...
function ruleLinePlacements(st, clues) {
  for (const line of eachSumsLine(st, clues)) {
    ...
    if (count === 0) {
      ...
      return { rule: escapeRejects ? 'Coral escape' : 'Line placements', contradiction: true, cells: line.cells.slice(),
    ...
    return { rule: escapeRejects ? 'Coral escape' : 'Line placements', cells: mkFilled.concat(mkBlank).map(p => line.cells[p]),
...
const SUMS_STRATEGIES = [
  ...
  { name: 'Coral escape', variant: 'blankConn', desc: 'A line\u2019s shaded cells come in stretches, and each stretch must join the coral through a blank-capable neighbour outside the line \u2014 arrangements that seal a stretch into a pocket are impossible.' },
  { name: 'Coral spine', variant: 'blankConn', desc: 'The one coral must place a shaded cell in every line whose clue forces one \u2014 blank-capable pockets that cannot reach such a line hold digits, and a lone bridge on every route between them is shaded.' },
