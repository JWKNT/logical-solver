...
function ruleContradiction(st, clues) {
    ...
    for (let i = 0; i < R * C; i++) if (st.cellCfg[i] === 0) {
        return {
            rule: 'Contradiction', contradiction: true, cells: [i],
    ...
    for (const line of eachLine(st, clues)) {
        ...
        if (line.clue.length >= 5 && line.clue.every(v => v >= 0)) {
            ...
            if (sum !== line.cells.length) {
                return {
                    rule: 'Contradiction', contradiction: true, cells: line.cells.slice(),
...
function ruleBranchParity(st, clues) {
    ...
    for (const line of eachLine(st, clues)) {
        ...
        if (F < 0) {
            return {
                rule: 'Branch orientation', contradiction: true, cells: line.cells.slice(),
        ...
        if (needEnd === 0 && candEnd.length) {
            return {
                rule: 'Branch orientation', cells: candEnd.slice(),
        ...
        if (needFlat === 0 && candFlat.length) {
            return {
                rule: 'Branch orientation', cells: candFlat.slice(),
    ...
    for (const line of eachLine(st, clues)) {
        for (const S of SHAPE_SUBSETS) {
            ...
            if (cand.length < demand || def.length > demand) {
                return {
                    rule: 'Combined shape count', contradiction: true, cells: line.cells.slice(),
            ...
            if (cand.length === demand && demand > 0) {
                ...
                if (targets.length) {
                    return {
                        rule: 'Combined shape count', cells: cand.slice(),
            ...
            if (def.length === demand) {
                ...
                if (targets.length) {
                    return {
                        rule: 'Combined shape count', cells: def.slice(),
            ...
            if (demand > 0 && cand.length > demand) {
                ...
                if (pairs.length) {
                    ...
                    if (maxAll === demand) {
                        ...
                        if (targets.length) {
                            return {
                                rule: 'Combined shape count', cells: forced.slice(),
...
function ruleExactAllocation(st, clues) {
    for (const line of eachLine(st, clues)) {
        ...
        return {
            rule: 'Exact allocation', cells: commitUsed.concat(commits.map(t2 => line.cells[t2.p])),
    ...
    for (const line of eachLine(st, clues)) {
        ...
        for (const b2 of nbBounds) {
            for (let s3 = 0; s3 < 4; s3++) {
                ...
                return {
                    rule: 'Paired exclusion', cells: commits.map(p => line.cells[p]),
        ...
        if (U >= 0) {
            ...
            if (baseMin > U) {
                return {
                    rule: 'Usage budget', contradiction: true, cells: line.cells.slice(),
            ...
            if (baseEnum.best > U) {
                return {
                    rule: 'Arm arrivals', contradiction: true, cells: line.cells.slice(),
            ...
            if (emptyForced.length) {
                ...
                if (firstT.en) {
                    return {
                        rule: 'Arm arrivals', cells: emptyForced.map(t => t.i),
                ...
                return {
                    rule: firstT.nb ? 'Neighbour arms' : (linkSup ? 'Escape routes' : 'Usage budget'), cells: emptyForced.map(t => t.i),
        ...
        if (usedForced.length) {
            ...
            return {
                rule: first.demandHit ? 'Piece demand' : 'Empty quota', cells: usedForced.map(t => t.i),
...
function ruleDerivedClue(st, clues) {
    ...
    const applyDerived = (line, s, B, why) => {
        ...
        if (def > B || pos < B) {
            return {
                rule: 'Derived clue', contradiction: true, cells: line.cells.slice(),
        ...
        if (def === B && pos > B) {
            return {
                rule: 'Derived clue', cells: extras,
        ...
        return {
            rule: 'Derived clue', cells: line.cells.slice(),
            text: why,
            apply() { pin(); }
        };
    };
...
function ruleUnreachable(st) {
    ...
    for (let g = 1; g < groups.length; g++) {
        if (find(groups[g][0]) !== home) {
            return {
                rule: 'Unreachable region', contradiction: true, cells: [groups[0][0], groups[g][0]],
...
function ruleSegmentRoom(st, clues) {
    ...
    for (const line of eachLine(st, clues)) {
        ...
        if (enders.length === 2) {
            ...
            if (p2 - p1 - 1 < k) {
                return {
                    rule: 'Segment room', contradiction: true, cells: [line.cells[p1], line.cells[p2]],
            ...
            if (!edges.length) continue;
            return {
                rule: 'Segment room', cells: [line.cells[p1], line.cells[p2]], edges: edges.map(e => [e[0], e[1]]),
        ...
        if (rightB === 1) dir = 1;
            ...
            if (!rOK && !lOK) {
                return {
                    rule: 'Segment room', contradiction: true, cells: [E],
        ...
        return {
            rule: 'Segment room', cells: [E], edges: edges.map(e => [e[0], e[1]]),
...
function ruleSegmentPacking(st, clues) {
    for (const line of eachLine(st, clues)) {
        ...
        if (info.contra) {
            return {
                rule: 'Segment packing', contradiction: true, cells: line.cells.slice(),
        ...
        return {
            rule: 'Segment packing', cells: [], edges: edges.map(e => [e[0], e[1]]),
...
function rulePerpParity(st, clues) {
    for (const line of eachLine(st, clues)) {
        ...
        if (candidates.length === 0 && parity === 1) {
            return {
                rule: 'Perpendicular-branch parity', contradiction: true, cells: line.cells.slice(),
        ...
        if (candidates.length === 1) {
            ...
            if (parity === 1) {
                return {
                    rule: 'Perpendicular-branch parity', cells: [i],
...
function ruleCountingLines(st, clues) {
    ...
    for (const kindName of ['row', 'col']) {
        ...
        for (let ri = 0; ri < group.length; ri++) {
            ...
            if (supply < D) {
                return {
                    rule: 'Counting lines', contradiction: true, cells: recv.cells.slice(),
            ...
            return {
                rule: 'Counting lines', cells: recv.cells.filter((i, p) => Wset.has(p)),
    ...
    for (const [kind, i] of cands) {
        ...
        for (const val of [0, 1]) {
            ...
            if (contra) {
                ...
                return {
                    rule: 'Border trial', cells: [], edges: [[kind, i]],
                    chain,
        ...
        for (let s = 0; s < 5; s++) {
            ...
            if (ghosts.length >= 2) {
                ...
                if (cellTargets.length || edgeTargets.length) {
                    ...
                    return {
                        rule: 'Case agreement', cells: cellTargets.map(t => t.i),
...
function ruleLineRecount(st, clues) {
    ...
    for (const line of eachLine(st, clues)) {
        ...
        for (let p = 0; p < line.cells.length; p++) {
            ...
            if (nm === 0) {
                return {
                    rule: 'Single-line analysis', contradiction: true, cells: [i],
        ...
        return {
            rule: 'Single-line analysis', cells: targets.map(t => t.i),
...
function ruleEntranceCounting(st, clues) {
    ...
    for (const d of dirs) {
        ...
        for (let m = 0; m < d.count; m++) {
            ...
            if (!any) {
                return {
                    rule: 'Entrance counting', contradiction: true, cells: [],
            ...
            if (edges.length) {
                ...
                return {
                    rule: 'Entrance counting', cells: [], edges: edges.map(e => [e[0], e[1]]),
            ...
            return {
                rule: 'Entrance counting', cells: targets.map(t => t.i),
